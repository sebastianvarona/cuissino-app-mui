    ...
    const handleClick = () => {
		...
		const Hamburguer = () => {
			if (!toggle) {
				...
				return <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 5l7 7-7 7M5 5l7 7-7 7"></path></svg>;
			}
		}

    return (
        
        <div className={`p-3 pt-6 fixed top-0 left-0 h-screen z-20 border-r dark:border-gray-800 border-dashed ${toggle? "" : "md:w-64 lg:w-72 md:px-6"}`}>

            {/* BRAND */}
            <div className="flex items-center justify-center cursor-default relative z-20 mb-16">
                <span className={`cuisiino-text hidden ${toggle? "" : "md:block"} text-4xl font-medium mr-1`}>Cuisiino</span>
                <img src="./icon2.svg" alt="cuisiino" className='h-9 cursor-pointer' />
            </div>

            {/* HAMBURGUER */}
            <div className={`hidden absolute h-20 md:flex items-center top-0 ${toggle? "left-20" : "left-20 md:left-64 lg:left-72"}`}>
